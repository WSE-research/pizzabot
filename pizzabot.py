# typing_extensions, not typing: pydantic needs its TypedDict on Python < 3.12,
# and LangGraph hands the state schema to pydantic.
from typing_extensions import TypedDict

from utils import logger, post_order, validate_pizza_name, check_customer_address, BasicFunctions, get_pizza_menu, get_delivery_area, check_order_intention, generate_pizza_description, call_qanary_pipeline
...
class CheckerNode:
    ...
    def invoke(self, state: ChatbotState) -> dict:
            ...
            if OrderSlots.PIZZA_NAME.value in state[MESSAGES][-1].content:
                ...
                if pizza_id is not None:  # if pizza name is valid
                    ...
                    state['messages'].append(AIMessage(
                        content="Invalid pizza type. Please specify a valid type (e.g. a pizza Pepperoni)'."))
                    return {
                        MESSAGES: state[MESSAGES],
                        "invalid": state["invalid"]
                    }

            # checking customer address validity
                ...
                if customer_address is not None:
                    ...
                    state["invalid"] = True
                    state['messages'].append(AIMessage(
                        content="Invalid customer address. Please keep in mind, we only deliver to " + get_delivery_area()))
                    return {
                        MESSAGES: state[MESSAGES],
                        "invalid": state["invalid"]
                    }
        ...
        if not check_order_intention(_input):  # User wants to order a pizza
            state['messages'].append(AIMessage(
                content="Invalid order. Please specify a pizza order. Try writing e.g. 'I want to order a pizza'."))
            return {
                MESSAGES: state[MESSAGES]
            }
...
class OrderNode:
    ...
    def invoke(self, state: ChatbotState) -> dict:
        ...
        required_slots = [OrderSlots.PIZZA_NAME,
                          OrderSlots.CUSTOMER_ADDRESS, OrderSlots.ORDER_ID]
        missing_slots = BasicFunctions.get_last_missing_slots(
            state, required_slots)

        if state["invalid"]:
            last_function_message = BasicFunctions.get_last_function_message(
                state)
            state["invalid"] = False
        ...
        if next_slot == OrderSlots.ORDER_ID.value:
            ...
            if order_id is not None:
                ...
                state['messages'].append(AIMessage(
                    content="Something went wrong while submitting your order, please try again."))
                state['ended'] = True
                return {
                    MESSAGES: state[MESSAGES],
                    "invalid": state["invalid"]
...
def build_graph():
    ...
    The one entry point every frontend uses -- console, Streamlit or test.
    Nobody else builds this graph, so a change here reaches all of them.
    """
...
def new_state(user_input: str = "") -> ChatbotState:
    """A fresh state -- every field exists from the start."""
    return {
        INPUT: user_input,
        SLOTS: {},
        MESSAGES: [],
...
if __name__ == "__main__":
    ...
    outputs = graph.invoke(new_state(user_input))
