...
|The kitchen view
|Seven tabs, the first five filled from the compiled graph. *Ticket*: the nodes that ran for the last sentence, in order, with their timings and the state keys each one wrote — read from `stream(..., stream_mode="updates")`, so it is what actually happened, not what should have. *Stations*: one card per node — what it does, where it routes, which service it calls, how it fails, where its code lives. *Order pad*: the state after the turn, filled values highlighted. *Floor plan*: the process model. *What happened*: the turn in plain language — the implementation's own account if it offers `what_happened(target)`, otherwise one assembled from the run and labelled as such. *LLM calls*: every prompt the turn sent to an LLM endpoint, with the answer, the response time and the tokens. *Test runs*: the example dialogue played against the loaded implementation, every run kept in `runs/` and compared in a table. A slider sets the pane's width, another the size of the floor plan.
...
The tab *LLM calls* lists every request the process sent to an LLM endpoint in this session, one turn at a time: pick the turn (the newest is selected after each sentence), and each call opens as a card with the model, the node that made it, the response time in milliseconds and — when the endpoint reports them — the prompt and answer tokens. Inside the card is the prompt as it went out, message by message with its role (`system`, `user`, `assistant`, …), and the answer that came back, or the error. The line above the picker adds up the calls and the seconds spent waiting for the endpoint.

The recording is as implementation-agnostic as the rest of the pane. `llm_log.Recorder` is handed to the graph as a LangChain callback — `graph.stream(state, config={"callbacks": [recorder]}, ...)` — so every chat model or LLM a node invokes through LangChain is seen by `on_chat_model_start` / `on_llm_start` / `on_llm_end` / `on_llm_error`. Most course implementations, the demo among them, call the endpoint through the `openai` client directly, which no callback ever hears of; for those, `create()` of the client's (chat) completions is wrapped once per process and records into the recorder of the turn that is running, and is a plain pass-through everywhere else. A LangChain model that uses the `openai` client underneath is recorded once, not twice. The node is read from LangGraph's run configuration, so the card says which station asked.

In offline mode (`PIZZABOT_OFFLINE=1`) the AI-backed steps run as rules and nothing reaches an endpoint; the tab says so instead of showing an empty list. The log lives in the session and keeps the last 60 turns; *New order* starts a new order number but keeps the log, a reload starts a new one. A streamed answer (`stream=True`) is recorded as the implementation reads it: the chunks pass through unchanged and a copy is kept, so nothing is read ahead; the text and any tool calls are put together from the chunks, the response time runs to the last one, and the tokens are shown when the request asked for them (`stream_options={"include_usage": True}`).

=== Test runs

The tab *Test runs* plays the example dialogue of `data/test_dialogue.py` — the one `test_pizzabot.py` evaluates through LangSmith — against the implementation behind the counter, and keeps every run for comparison. No LangSmith account, no judge model and no change to the implementation are needed.

*Run the test dialogue on &lt;key&gt;* sends the utterances of the dialogue one after the other into a fresh state of the implementation's own (`new_state()`), beside the conversation at the counter, which stays as it is. The recorded input states of the file are not used: they carry the demo's state keys, which another implementation does not have, while the utterances and the expected answers are what every implementation shares. A turn *passes* when the expected answer is contained in what the bot said in that turn, compared case- and whitespace-insensitively, with placeholders such as `<SOME UUID>` matching anything — the question `test_pizzabot.py` puts to its LLM judge (“does the actual answer contain all of the information in the expected answer?”), asked without a model. The similarity shown beside it (difflib's ratio, 0 to 1) says how far off a failed turn is.

Every run is stored as one JSON file in `runs/` (ignored by git; `PIZZABOT_RUNS` in `.env` moves it, relative to this folder or absolute), named `<started>-<key>.json`: when it ran, the implementation's key, title and module, its git revision when its folder is a checkout (`+` for uncommitted changes), offline mode or the model, and per turn the utterance, the expected and the actual answer, pass, fail or skipped, the similarity, the milliseconds, the nodes that ran, the number of LLM calls and the milliseconds they took, and an error if the turn raised. The table under the button compares all stored runs, newest first and across implementations — when, implementation, mode (`offline · rules` or the model name), passed/graded turns and how many were skipped, duration, LLM calls, revision — with the run just made highlighted; the picker under it opens the turns of any run.

The last turn of the dialogue completes the order, and a working implementation then places a *real* order with the Pizza API, as `test_pizzabot.py` does. That is why the checkbox *Place the real order* above the button is off by default: the last utterance is then not sent, the turn is stored as skipped (`"skipped": true`) and left out of the verdict, so the table reads `2/2 · 1 skipped`. Tick it to play the whole dialogue. Which turn is skipped follows from the dialogue, not from the implementation — it is the last one — so this works for every implementation alike; the run records the choice as `place_order`.

The dialogue was written for the demo, so another implementation failing it says that its wording differs, which is precisely what a comparison across implementations is meant to show.
...
llm_log.py            records the LLM calls of a turn, for any implementation
test_runs.py          plays the example dialogue against an implementation, keeps the runs
...
PIZZABOT_APP=demo
PIZZABOT_RUNS=runs        # where the test runs are kept (optional)
...
|`pz-kitchen`, `pz-kitchen-width`, `pz-widget-kitchen-width`, `pz-kitchen-tabs`, `pz-tab-ticket`, `pz-tab-stations`, `pz-tab-order-pad`, `pz-tab-floor-plan`, `pz-floor-plan-size`, `pz-tab-what-happened`, `pz-tab-llm-calls`, `pz-tab-test-runs`, `pz-button-run-tests`, `pz-widget-place-order` |the kitchen pane, its width control, its tabs and the floor plan's size control
