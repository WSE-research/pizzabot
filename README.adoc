...
In offline mode (`PIZZABOT_OFFLINE=1`) the AI-backed steps run as rules and nothing reaches an endpoint; the tab says so instead of showing an empty list. The log lives in the session and keeps the last 60 turns; *New order* starts a new order number but keeps the log, a reload starts a new one. Streamed answers are recorded with their prompt and timing, but the answer text is not read, so as not to consume the stream the implementation is waiting for.
