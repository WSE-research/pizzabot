...
Every run is stored as one JSON file in `runs/` (ignored by git; `PIZZABOT_RUNS` in `.env` moves it, relative to this folder or absolute), named `<started>-<key>.json`: when it ran, the implementation's key, title and module, its git revision when its folder is a checkout (`+` for uncommitted changes), offline mode or the model, and per turn the utterance, the expected and the actual answer, pass or fail, the similarity, the milliseconds, the nodes that ran, the LLM calls and an error if the turn raised. The table under the button compares all stored runs, newest first and across implementations — when, implementation, mode (`offline · rules` or the model name), passed/total, duration, LLM calls, revision — with the run just made highlighted; the picker under it opens the turns of any run.

Two things to know: the last turn of the dialogue places a real order with the Pizza API, exactly as `test_pizzabot.py` does; and the dialogue was written for the demo, so another implementation failing it says that its wording differs, which is precisely what a comparison across implementations is meant to show.
...
|`pz-kitchen`, `pz-kitchen-width`, `pz-widget-kitchen-width`, `pz-kitchen-tabs`, `pz-tab-ticket`, `pz-tab-stations`, `pz-tab-order-pad`, `pz-tab-floor-plan`, `pz-floor-plan-size`, `pz-tab-what-happened`, `pz-tab-llm-calls`, `pz-tab-test-runs`, `pz-button-run-tests` |the kitchen pane, its width control, its tabs and the floor plan's size control
