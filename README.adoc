...
++++
<a href="https://wse-research.org/?utm_source=pizzabot&utm_medium=readme&utm_campaign=pizzabot&utm_content=logo" title="go to WSE Research">
<img align="right" role="right" height="128" src="assets/wse-logo.png"/>
</a>
++++
...
* *Runs with no LLM at all* — `PIZZABOT_OFFLINE=1` swaps each AI-backed step for a deterministic rule of the same signature, so a demo survives a room with no VPN and a course key that has run out.
...
MODEL_NAME=the model that endpoint serves
...
`OPENAI_API_BASE` is any OpenAI-compatible endpoint — the HTWK GPU server inside the university network, a hosted service, or a local one. Query its `/models` for the name to put in `MODEL_NAME`. A shell `PIZZABOT_OFFLINE` wins over the file, which is how `run_local.sh` reports its probe; for everything else the file wins.

`PIZZA_API_BASE` has a default, so a fresh checkout answers *what pizzas do you have?* before anyone has written a `.env`. Credentials have no such default: a missing key stays a missing key, and says so.
...
We are happy to receive your contributions. Please create a pull request or an {github-repository}/issues/new[issue], and feel free to {github-repository}/fork[fork] it.
